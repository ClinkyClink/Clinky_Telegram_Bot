...
def edit_button(item_id):
    return InlineKeyboardButton(text='Редактировать', callback_data=f'edit_item_{item_id}')
...
def edit_item_keyboard(item_id):
    keyboard = InlineKeyboardBuilder()
    keyboard.row(
        InlineKeyboardButton(text='Категория', callback_data=f'edit_field_category_{item_id}'),
        InlineKeyboardButton(text='Документация', callback_data=f'edit_field_documentation_{item_id}')
    )
    keyboard.row(
